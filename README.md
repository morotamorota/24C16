...
- 日付の形式が正しくない場合、エラーメッセージが表示されます。
- データベースに該当するデータが存在しない場合、メッセージが表示されます。

### 結果の出力
- 取得したデータは、指定されたシート名で新しいシートとして作成され、テーブル形式で表示されます。
- テーブルスタイルは`TableStyleLight9`に設定されています。

## 必要な環境
- Excel VBA
- ODBC接続を使用したデータベースアクセス
